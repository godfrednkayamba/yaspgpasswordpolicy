## yaspgpp - PostgreSQL username/password checks

- [yaspgpp - PostgreSQL username/password checks](#yaspgpp---postgresql-usernamepassword-checks)
...
The `yaspgpp` PostgreSQL extension provides few general credential checks, which will be evaluated during the user creation, during the password change and user renaming. By using this extension, we can define a set of rules:
...
This extension provides all the checks as configurable parameters. The default configuration settings, will not enforce any complex checks and will try to allow most of the credentials. By using `SET yaspgpp.<check-name> TO <some value>;` command, enforce new settings for the credential checks. The settings can only be changed by a superuser.

### [Installation](#installation)

To install the yaspgpp extension you need a PostgreSQL version upper than 10
...
Append `yaspgpp` to `shared_preload_libraries` configuration parameter in your
...
There is also the `yaspgpp.whitelist` GUC that can be used to set a comma separated list of username to exclude from the password policy check. For example:
```
yaspgpp.whitelist = 'admin,supuser'
```
will disable any yaspgpp policy for the user named `admin` and  `supuser`.
...
```
postgres=# SHOW yaspgpp.username_min_length;
 yaspgpp.username_min_length 
-------------------------------
 4
(1 row)

postgres=# CREATE USER abc WITH PASSWORD 'pass';
ERROR:  username length should match the configured yaspgpp.username_min_length
...
```
postgres=# SHOW yaspgpp.username_min_special;
 yaspgpp.username_min_special 
--------------------------------
 1
(1 row)

postgres=# CREATE USER abcd WITH PASSWORD 'pass';
ERROR:  username does not contain the configured yaspgpp.username_min_special characters
...
```
postgres=# show yaspgpp.username_min_repeat ;
 yaspgpp.username_min_repeat 
-------------------------------
 1
(1 row)

postgres=# CREATE USER week$ WITH PASSWORD 'pass';
ERROR:  username characters are repeated more than the configured yaspgpp.username_min_repeat times
...
```
postgres=# SHOW yaspgpp.password_not_contain ;
...
ERROR:  password does contain the configured yaspgpp.password_not_contain characters
...
```
postgres=# SHOW yaspgpp.password_contain_username ;
 yaspgpp.password_contain_username 
-------------------------------------
 on
(1 row)
...
```
postgres=# SHOW yaspgpp.password_ignore_case;
 yaspgpp.password_ignore_case 
--------------------------------
 on
(1 row)
...
```
postgres=# SHOW yaspgpp.password_min_repeat ;
 yaspgpp.password_min_repeat 
-------------------------------
 3
(1 row)

postgres=# CREATE USER abcd$ WITH PASSWORD 'straaaangepaasssword';
ERROR:  password characters are repeated more than the configured yaspgpp.password_min_repeat times

postgres=# CREATE USER abcd$ WITH PASSWORD 'straaangepaasssword';
CREATE ROLE
```

yaspgpp can also enforce the use of an expiration date for the password by checking option VALID UNTIL used in CREATE or ALTER ROLE.
```
postgres=# SET yaspgpp.password_valid_until = 30;
SET

postgres=# SET yaspgpp.password_valid_max = 180;
SET
...
With the yaspgpp extension, password can be forced to be of a certain length, contain amounts of various types of characters and be checked against the user account name itself.
...
The yaspgpp extension adds the "Password Reuse Policy" in release 1.0. To used this feature, the yaspgpp extension MUST be added to `shared_preload_libraries` configuration option.
...
The password history size is set to 65535 records by default and can be adjusted using the `yaspgpp.history_max_size` configuration directive. Change of this GUC require a PostgreSQL restart. One record in the history takes 144 bytes, so the default is to allocate around 10 MB of additional shared memory for the password history.

Two settings allow to control the behavior of this feature:

* `yaspgpp.password_reuse_history`: number of distinct passwords set before a password can be reused.
* `yaspgpp.password_reuse_interval`: amount of time it takes before a password can be reused again.

The default value for these settings are 0 which means that all password reuse policies are disabled.

The password history consists of passwords a user has been assigned in the past. yaspgpp can
...
the yaspgpp extension. The view is named `public.pg_password_history`. This view is visible by everyone.
...
PostgreSQL doesn't have any mechanism to limit the number of authentication failure attempt before the user being banned.  With the yaspgpp extension, after an amount of authentication failure defined by configuration directive `yaspgpp.max_auth_failure` the user can be banned and never connect anymore even if it gives the right password later.

The yaspgpp extension adds the "Authentication failure ban" feature in release 2.0. To used this feature, the yaspgpp extension MUST be added to `shared_preload_libraries` configuration option.
...
The authentication failure cache size is set to 1024 records by default and can be adjusted using the `yaspgpp.auth_failure_cache_size` configuration directive. Change of this GUC require a PostgreSQL restart.

Two settings allow to control the behavior of this feature:

* `yaspgpp.max_auth_failure`: number of authentication failure allowed for a user before being banned.
* `yaspgpp.reset_superuser` : force superuser to not be banned or reset a banned superuser when set to true.
...
In case the `postgres` superuser was banned, he can not logged anymore. If there is no other superuser account that can be used to reset the record of the banned superuser, set the `yaspgpp.reset_superuser`configuration directive to `true` into postgresql.conf file and send the SIGHUP signal to the PostgreSQL process pid so that it will reread the configuration. Next time the superuser will try to connect, its authentication failure cache entry will be removed.
...
```
	yaspgpp.whitelist_auth_failure = 'appuser1,appuser2'
```

### [Authentication delay](#authentication-delay)

This feature allow a pause on authentication failure. Setting `yaspgpp.auth_delay_ms` causes the server to pause for a given number of milliseconds before reporting authentication failure. This makes brute-force attacks on database passwords more difficult. 
...
variable `yaspgpp.encrypted_password_allowed`.
